            ...
            case 'update_receipt':
                ...
                try {
                    ...
                    foreach ($items as $itemData) {
                        ...
                        $packageWeight = floatval($itemData['package_weight_per_unit'] ?? 0);
                        $receivedPackageQty = floatval($itemData['quantity_received']); // จำนวนกระป๋อง
                        
                        // ✅ คำนวณ quantity_received (กิโล) จากจำนวนกระป๋อง
                        if ($packageWeight > 0) {
                            $newQty = $receivedPackageQty * $packageWeight; // กระป๋อง × กิโล/กระป๋อง = กิโล
                        } else {
                            $newQty = $receivedPackageQty; // ถ้าไม่มี package_weight ให้ใช้ค่าเดิม
                        }
                        ...
                        if ($locationChanged) {
                            ...
                            if ($qtyDiff != 0) {
                                ...
                                // ✅ คำนวณ package qty diff
                                $packageQtyDiff = 0;
                                if ($packageWeight > 0) {
                                    $oldPackageQty = $oldQty / $packageWeight;
                                    $newPackageQty = $newQty / $packageWeight;
                                    $packageQtyDiff = abs($newPackageQty - $oldPackageQty);
                                }
                                ...
                                $stmt->execute([
                                    ...
                                    $userId,
                                    $notes,
                                    $packageWeight > 0 ? $packageQtyDiff : null,  // received_package_qty
                                    $packageWeight > 0 ? $packageWeight : null     // package_weight_per_unit
                                ]);
                            }
                        }
                    }
...
$stmt = $pdo->prepare("
    SELECT 
        ...
        ist.current_stock,
        ist.available_stock,
        -- ✅ ดึงจำนวนกระป๋องจาก Stock_Movements
        (SELECT SUM(CASE 
        ...
        WHERE sm.product_id = gri.product_id
          ...
          AND sm.reference_type = 'GR'
          AND sm.reference_id = gri.gr_item_id
          AND sm.received_package_qty IS NOT NULL
          AND sm.received_package_qty != 0
        ) as received_package_qty,
        -- ✅ ดึงน้ำหนักต่อกระป๋อง
        (SELECT TOP 1 sm.package_weight_per_unit
        FROM Stock_Movements sm
        WHERE sm.product_id = gri.product_id
          AND sm.warehouse_id = gri.warehouse_id
          AND sm.location_id = gri.location_id
          AND sm.reference_type = 'GR'
          AND sm.reference_id = gri.gr_item_id
          AND sm.package_weight_per_unit IS NOT NULL
          AND sm.package_weight_per_unit != 0
        ORDER BY sm.movement_date DESC
        ) as package_weight_per_unit
    FROM Goods_Receipt_Items gri
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="text-align: right;">
                                <strong><?php echo number_format($item['received_package_qty'] ?? $item['quantity_received'], 2); ?></strong>
                    ...
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><?php echo htmlspecialchars($item['product_name']); ?></td>
                                <td>
                                    <input type="number" class="old-quantity" value="<?php echo $item['received_package_qty'] ?? $item['quantity_received']; ?>" disabled>
                                </td>
                                <td>
                                    <input type="number" class="new-quantity" step="0.01" min="0" 
                                           value="<?php echo $item['received_package_qty'] ?? $item['quantity_received']; ?>" required>
                                </td>
                                <td>
    ...
    <script>
        ...
        document.getElementById('editForm').addEventListener('submit', async function(e) {
            ...
            document.querySelectorAll('tbody tr').forEach(row => {
                const packageWeight = parseFloat(row.querySelector('.package-weight').value) || 0;
                const newQty = parseFloat(row.querySelector('.new-quantity').value) || 0;
                ...
                items.push({
                    ...
                    quantity_received: newQty,  // จำนวนกระป๋อง
                    package_weight_per_unit: packageWeight,  // น้ำหนักต่อกระป๋อง
