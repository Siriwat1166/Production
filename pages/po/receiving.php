...
$mode = (isset($_GET['mode']) && in_array($_GET['mode'], ['po','direct'])) ? $_GET['mode'] : 'po';
...
case 'calculate_conversion':
    ...
    break;
            ...
            case 'save_receipt_enhanced':
                ob_clean();
                echo json_encode(saveReceiptEnhanced($_POST));
                break;
...
function saveReceiptEnhanced($data) {
    ...
    try {
        ...
        $pdo->beginTransaction();
        // TODO: ใส่ logic บันทึก GR Header/Items + Movement + Lot Tracking ที่ใช้กับ PO
        $pdo->commit();
        return ['success' => true, 'message' => 'Receipt saved successfully', 'gr_number' => 'GR-TEST-001'];
    } catch (Exception $e) {
        if ($pdo && $pdo->inTransaction()) $pdo->rollback();
...
.navbar{background:rgba(139,69,19,.9);box-shadow:0 4px 20px rgba(139,69,19,.3)}
...
.mode-selector{background:rgba(255,255,255,.95);border-radius:15px;padding:20px;margin-bottom:20px;box-shadow:0 4px 15px rgba(139,69,19,.1)}
.mode-btn{border:2px solid rgba(139,69,19,.3);border-radius:10px;padding:15px;transition:all .3s ease;background:#fff}
.mode-btn:hover{border-color:var(--primary-color);transform:translateY(-2px)}
.mode-btn.active{background:var(--primary-gradient);color:#fff!important;border-color:var(--primary-color)}
.mode-btn.active h5,.mode-btn.active small{color:#fff!important}
.sticky-actions{position:sticky;bottom:0;background:rgba(255,255,255,.98);padding:20px;box-shadow:0 -10px 30px rgba(139,69,19,.1);border-radius:0 0 20px 20px}
...
<nav class="navbar navbar-expand-lg navbar-dark">
  <div class="container-fluid">
    <div class="d-flex align-items-center">
      <a href="/PD/production/inventory/inventory_view.php" class="btn btn-link text-white me-3"><i class="fas fa-arrow-left"></i></a>
      <div>
        <h5 class="navbar-brand mb-0"><i class="fas fa-clipboard-check me-2"></i>รับเข้าสินค้า</h5>
        <small class="text-light">ระบบจัดการรับเข้าสินค้า</small>
      </div>
    </div>
    <div class="navbar-nav ms-auto">
      <a class="nav-link" href="../dashboard.php"><i class="fas fa-th-large me-1"></i> รายการรอรับ</a>
      <a class="nav-link" href="#"><i class="fas fa-user-circle me-1"></i> <?= h($_SESSION['full_name'] ?? 'ผู้ใช้งาน'); ?></a>
    </div>
  </div>
</nav>
...
<div class="container py-4" style="max-width:1400px;">
  ...
  <div class="mode-selector">
    <div class="row g-3">
      <div class="col-md-6">
        <button type="button" class="mode-btn btn w-100 active" id="btnPOMode" onclick="switchReceiptMode('po')">
          <i class="fas fa-file-invoice fa-2x mb-2"></i><h5 class="mb-0">รับเข้าจาก PO</h5><small>รับสินค้าตามใบสั่งซื้อ</small>
        </button>
      </div>
      <div class="col-md-6">
        <button type="button" class="mode-btn btn w-100" id="btnDirectMode" onclick="switchReceiptMode('direct')">
          <i class="fas fa-truck-loading fa-2x mb-2"></i><h5 class="mb-0">รับเข้าโดยตรง (ไม่มี PO)</h5><small>รับสินค้าโดยไม่ผ่านใบสั่งซื้อ</small>
        </button>
      </div>
    </div>
  </div>
  ...
  <!-- ===================== DIRECT MODE ===================== -->
  <div id="directModeSection" style="display:<?= ($mode==="direct"?"block":"none") ?>;">
    <div class="card mb-4">
    ...
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        ...
        <button type="button" class="btn btn-light btn-sm" id="btnAddCard"><i class="fas fa-plus me-1"></i>เพิ่มรายการ</button>
      </div>
      <div class="card-body">
        <div id="itemsCards" class="d-flex flex-column"></div>
      </div>
      <div class="sticky-actions d-flex gap-3 justify-content-end">
        <button type="button" class="btn btn-outline-secondary" onclick="cancelDirectReceipt()"><i class="fas fa-times me-1"></i>ยกเลิก</button>
        <button type="button" class="btn btn-success" id="btnSaveDraft"><i class="fas fa-save me-1"></i>บันทึกใบรับเข้า</button>
      </div>
    </div>
  </div>
</div>
...
function recalcLotSum(itemIdx){
  ...
  if (sumEl) {
    sumEl.textContent = sum.toLocaleString('th-TH', {minimumFractionDigits: 2, maximumFractionDigits: 4});
  }
  if (palletSumEl) {
    palletSumEl.textContent = pallet.toLocaleString('th-TH');
  }
  ...
  if (sum > maxQty && maxQty > 0) {
    ...
    if (sumEl) {
      ...
      sumEl.innerHTML = `${sum.toLocaleString('th-TH', {minimumFractionDigits: 2})} <i class="fa fa-exclamation-triangle ms-1" title="เกินจำนวนที่รับ ${diff.toFixed(2)}"></i>`;
    }
    ...
    if (!warningBadge) {
      ...
      warningBadge.innerHTML = `
        ...
        รวม: <span class="fw-bold">${sum.toFixed(2)}</span> 
        | ต้องรับ: <span class="fw-bold">${maxQty.toFixed(2)}</span> 
        | เกิน: <span class="fw-bold text-danger">${diff.toFixed(2)}</span>
      `;
    ...
    const remaining = maxQty - sum;
    
    // แสดงเตือนว่ายังขาด
    if (sumEl) {
      sumEl.style.color = '#f59e0b';
      sumEl.style.fontWeight = 'bold';
      sumEl.innerHTML = `${sum.toLocaleString('th-TH', {minimumFractionDigits: 2})} <i class="fa fa-info-circle ms-1" title="ยังขาด ${remaining.toFixed(2)}"></i>`;
    }
    ...
    if (!warningBadge) {
      ...
      warningBadge.innerHTML = `
        ...
        รวม: <span class="fw-bold">${sum.toFixed(2)}</span> 
        | ต้องรับ: <span class="fw-bold">${maxQty.toFixed(2)}</span> 
        | ยังขาด: <span class="fw-bold text-warning">${remaining.toFixed(2)}</span>
      `;
      const lotStrip = card?.querySelector('.lot-strip');
      if (lotStrip) {
        lotStrip.insertAdjacentElement('afterend', warningBadge);
    ...
    if (sumEl) {
      ...
      sumEl.style.fontWeight = 'bold';
      sumEl.innerHTML = `${sum.toLocaleString('th-TH', {minimumFractionDigits: 2})} <i class="fa fa-check-circle ms-1 text-success" title="ครบถ้วน"></i>`;
    }
  ...
  console.log(`Item ${itemIdx} - Lot sum: ${sum}, Max: ${maxQty}, Pallet: ${pallet}`);
}
        ...
        <div class="po-item-head" style="display:flex;gap:16px;align-items:flex-start;justify-content:space-between">
          ...
          <div style="min-width:450px;background:#f8f9fa;padding:15px;border-radius:10px">
            ...
            <div class="row g-2 align-items-center">
              ...
              <div class="col-8">
                <input class="form-control text-end recv-qty fw-bold" name="recv_qty_${idx}" placeholder="0.0000" step="0.0001" min="0" style="font-size:1.2rem;color:#059669;background:white" />
            ...
            <tfoot>
              <tr class="table-light">
                ...
                <td colspan="5" class="text-end">
                  <small class="text-primary fw-bold">
                    <i class="fa fa-clipboard-check me-1"></i>ต้องรับ: ${needQty > 0 ? needQty.toLocaleString() : orderQty.toLocaleString()} ${hEsc(purchaseUnitSymbol)}
                  </small>
                </td>
              </tr>
            </tfoot>
      ...
      if (purchaseUnitId && orderQty > 0) {
        ...
        try {
          ...
          if (convertedQty !== null && convertedQty > 0) {
            // แสดงจำนวนที่แปลงได้
            qtyEl.value = convertedQty.toFixed(4);
            
            // เติมลงล็อตแรกทันที
            const firstLot = rows[0]?.querySelector('.lot-qty');
            if (firstLot) {
              firstLot.value = convertedQty.toFixed(4);
              recalcLotSum(idx);
            }
    ...
    // ✅ กรณีที่ 2: มีค่าอยู่แล้ว และเปลี่ยนหน่วย - แปลงค่าเดิม
      ...
      try {
        ...
        if (convertedHeader !== null && convertedHeader > 0) {
          qtyEl.value = convertedHeader.toFixed(4);
        }
...
// ===== Payload รวบรวมข้อมูลจากการ์ด (PO Mode) =====
function collectItemPayload(idx, item){
  ...
  card.querySelectorAll('#lot_body_'+idx+' tr').forEach(tr=>{
    lots.push({
      ...
      pallet: parseFloat(tr.querySelector('.lot-pallet')?.value || '0') || 0,
...
function submitReceiptEnhancedPO(){
  ...
  showLoading();
  callApi('save_receipt_enhanced', {
    po_data: JSON.stringify({...window.currentPOData, warehouse_id}),
    items_data: JSON.stringify(items_data),
    receipt_date, invoice_no, notes
  }).then(res=>{
    hideLoading();
    if (res?.success){
      alert('บันทึกสำเร็จ: ' + (res.gr_number || ''));
      // TODO: redirect
    } else {
