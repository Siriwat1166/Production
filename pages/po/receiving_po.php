...
function getWarehouses() {
    global $pdo;
    try {
        // ✅ แก้ไข: รองรับทั้งตัวพิมพ์ใหญ่-เล็ก และชื่อที่หลากหลาย
        $query = "SELECT 
                    warehouse_id, 
                    warehouse_code, 
                    warehouse_name, 
                    ...
                    address,
                    CASE 
                        WHEN UPPER(warehouse_code) = 'MAIN' OR warehouse_code LIKE '%MAIN%' THEN 'คลังหลัก'
                        WHEN UPPER(warehouse_code) = 'HOLD' OR warehouse_code LIKE '%HOLD%' THEN 'คลังพักสินค้า'
                        ELSE COALESCE(warehouse_name_th, warehouse_name)
                    END AS display_name
                  FROM Warehouses 
                  WHERE is_active = 1 
                    AND (
                        UPPER(warehouse_code) IN ('MAIN', 'HOLD')
                        OR warehouse_code LIKE '%MAIN%'
                        OR warehouse_code LIKE '%HOLD%'
                        OR UPPER(warehouse_name) LIKE '%MAIN%'
                        OR UPPER(warehouse_name) LIKE '%HOLD%'
                    )
                  ORDER BY 
                    CASE 
                        WHEN UPPER(warehouse_code) = 'MAIN' OR warehouse_code LIKE '%MAIN%' THEN 1 
                        WHEN UPPER(warehouse_code) = 'HOLD' OR warehouse_code LIKE '%HOLD%' THEN 2 
                        ELSE 3 
                    END";
        $stmt = $pdo->prepare($query);
        $stmt->execute();
...
// 🔥 เพิ่ม 2 columns สำหรับ Package Tracking
// 7. Insert Stock_Movements
...
// 🔥 แก้ไข: ดึงจาก $item แทน $_POST
// 🔥 แก้ไข: ดึงจาก $item แทน $_POST
$receive_package_qty = null;
$weight_per_package = null;

if (isset($item['recv_package_qty']) && $item['recv_package_qty'] > 0) {
    $receive_package_qty = floatval($item['recv_package_qty']);
    
    if (isset($item['po_item_id']) && $item['po_item_id']) {
        $wpStmt = $pdo->prepare("
            SELECT weight_per_package 
            FROM PO_Items 
            WHERE po_item_id = ?
        ");
        $wpStmt->execute([$item['po_item_id']]);
        $weight_per_package = $wpStmt->fetchColumn();
        $wpStmt->closeCursor();
    }
}
...
$movementParams = [
    ...
    $loc['note'] ?? null,
    $loc['pallet_count'] ?? 0,
    $receive_package_qty,      // 🔥 เพิ่ม
    $weight_per_package        // 🔥 เพิ่ม
];
           ...
           // 8. Update PO_Items received quantity
            if (isset($item['po_item_id']) && $item['po_item_id']) {
                // ✅ ดึงข้อมูล PO_Items เพื่อเช็คหน่วย
                $poCheckStmt = $pdo->prepare("
                    SELECT purchase_unit_id, stock_unit_id, conversion_factor 
                    FROM PO_Items 
                    WHERE po_item_id = ?
                ");
                $poCheckStmt->execute([$item['po_item_id']]);
                $poCheck = $poCheckStmt->fetch(PDO::FETCH_ASSOC);
                $poCheckStmt->closeCursor();
                
                // ✅ คำนวณจำนวนที่จะบันทึก ต้องเป็นหน่วย purchase_unit
                $receiveQtyForPO = $item['receive_qty'];
                
                if ($poCheck) {
                    error_log("PO Unit Check - PO Item ID: {$item['po_item_id']}");
                    error_log("  Purchase Unit ID: {$poCheck['purchase_unit_id']}");
                    error_log("  Stock Unit ID: {$poCheck['stock_unit_id']}");
                    error_log("  Received in Unit ID: {$item['unit_id']}");
                    error_log("  Receive Qty: {$item['receive_qty']}");
                    error_log("  Conversion Factor: {$poCheck['conversion_factor']}");
                    
                    // ✅ ถ้าหน่วยที่รับไม่ตรงกับหน่วยซื้อ
                    if (isset($item['unit_id']) && $item['unit_id'] != $poCheck['purchase_unit_id']) {
                        
                        // กรณีที่ 1: รับเป็น stock_unit แต่ PO เป็น purchase_unit
                        if ($item['unit_id'] == $poCheck['stock_unit_id'] && $poCheck['conversion_factor'] > 0) {
                            // แปลงจาก stock → purchase (เช่น kg → แผ่น)
                            $receiveQtyForPO = $item['receive_qty'] / $poCheck['conversion_factor'];
                            error_log("  ✅ Converted: {$item['receive_qty']} stock → {$receiveQtyForPO} purchase");
                        }
                        // กรณีที่ 2: รับเป็น purchase_unit แต่ PO เป็น stock_unit (ไม่น่าจะเกิด แต่ป้องกันไว้)
                        else if ($item['unit_id'] == $poCheck['purchase_unit_id'] && $poCheck['conversion_factor'] > 0) {
                            // ใช้ตรงๆ (เพราะตรงกันแล้ว)
                            $receiveQtyForPO = $item['receive_qty'];
                            error_log("  ✅ No conversion needed (already in purchase unit)");
                        }
                        else {
                            // กรณีอื่นๆ - เตือน
                            error_log("  ⚠️ WARNING: Unknown unit conversion scenario!");
                        }
                    } else {
                        error_log("  ✅ Units match - no conversion needed");
                    }
                }
                
                error_log("  Final receiveQtyForPO: {$receiveQtyForPO}");
                
                // ✅ อัพเดทด้วยจำนวนที่แปลงแล้ว
                $sql = "UPDATE PO_Items 
                ...
                $stmt = $pdo->prepare($sql);
                if (!$stmt->execute([
                    $receiveQtyForPO,  // ✅ ใช้ค่าที่แปลงแล้ว
                    $receiveQtyForPO, 
                    $receiveQtyForPO, 
                    $receiveQtyForPO, 
                    $receiveQtyForPO, 
                ...
                error_log("✅ PO_Items updated successfully");
            }
        }
...
/* ✅ Select2 styling for lot-table */
.lot-table .select2-container {
  width: 100% !important;
}
.lot-table .select2-selection {
  height: auto !important;
  min-height: 38px;
  padding: 0.35rem 0.5rem !important;
  font-size: 0.875rem;
}
.lot-table .select2-selection__rendered {
  line-height: 1.5;
  padding: 0 !important;
}
.lot-table .select2-selection__arrow {
...
/* 🔥 สไตล์สำหรับสินค้าที่รับครบแล้ว */
.po-item-card.fully-received {
  position: relative;
...
.po-item-card.fully-received input,
.po-item-card.fully-received select,
.po-item-card.fully-received button {
  pointer-events: none;
  opacity: 0.6;
  background-color: #f0fdf4 !important;
}

.po-item-card.fully-received .btn {
  display: none; /* ซ่อนปุ่มทั้งหมด */
}

.po-item-card.fully-received::after {
  content: '';
  position: absolute;
  top: 0;
  left: 0;
...
.po-item-card.fully-received .po-item-head {
  position: relative;
  z-index: 2;
}
</style>
  ...
  <div id="poModeSection" style="display:<?= ($mode==="po"?"block":"none") ?>;">
          ...
          <div class="col-md-4">
            ...
            <select class="form-select" id="warehouseSelect" required>
              ...
              <?php foreach ($Warehouses as $w): ?>
                <option value="<?= (int)$w['warehouse_id']; ?>" data-code="<?= h($w['warehouse_code']); ?>">
                  <?= h($w['display_name']); ?>
                </option>
              <?php endforeach; ?>
            </select>
    ...
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        <span><i class="fas fa-boxes me-2"></i>รายการสินค้า</span>
        <div id="itemSelectionButtons" style="display: none;">
          <button type="button" class="btn btn-sm btn-outline-success me-2" onclick="selectAllItems()">
            <i class="fas fa-check-square me-1"></i>เลือกทั้งหมด
          </button>
          <button type="button" class="btn btn-sm btn-outline-danger" onclick="unselectAllItems()">
            <i class="fas fa-square me-1"></i>ยกเลิกทั้งหมด
          </button>
        </div>
      </div>
      <div class="card-body">
...
function fillLocationOptions(selectEl){
  ...
  // ✅ Initialize Select2 พร้อมการค้นหา
  $(selectEl).select2({
    theme: 'bootstrap-5',
...
function toggleItemReceiving(itemIdx) {
  const checkbox = document.getElementById(`select_item_${itemIdx}`);
  ...
  // ปรับสีและความชัดเจนของการ์ด
  if (isSelected) {
    card.style.opacity = '1';
    card.style.filter = 'none';
    card.style.borderColor = 'rgba(139,69,19,.1)';
  } else {
    card.style.opacity = '0.5';
    card.style.filter = 'grayscale(50%)';
    card.style.borderColor = '#ccc';
    
    // ล้างค่าจำนวนรับและล็อต
    const recvQtyInput = card.querySelector(`[name="recv_qty_${itemIdx}"]`);
    if (recvQtyInput) recvQtyInput.value = '';
    
    const lotBody = document.getElementById(`lot_body_${itemIdx}`);
    if (lotBody) {
      lotBody.querySelectorAll('.lot-qty, .lot-qty-calculated, .lot-package-qty').forEach(input => {
        input.value = '0';
      });
  ...
  console.log(`Item ${itemIdx} ${isSelected ? 'selected' : 'unselected'} for receiving`);
}

function selectAllItems() {
  const checkboxes = document.querySelectorAll('.item-select-checkbox:not(:disabled)');
  checkboxes.forEach(cb => {
    if (!cb.checked) {
      cb.checked = true;
      const itemIdx = parseInt(cb.dataset.item);
      toggleItemReceiving(itemIdx);
    }
  });
...
function renderLotRow(itemIdx, rowIdx){
  ...
  // 🔥 ดึงข้อมูล package จาก item
  const item = window.currentItems?.[itemIdx];
  const hasPackage = item?.package_qty && item?.weight_per_package;
  const weightPerPackage = item?.weight_per_package || 0;
  ...
  return `
    <tr data-item="${itemIdx}" data-row="${rowIdx}">
      ...
      ${hasPackage ? `
  <!-- 🔥 แสดงฟิลด์จำนวนกระป๋อง ถ้ามี package -->
  <td>
    <input type="number" 
           class="form-control text-end lot-package-qty" 
           name="lot_package_qty_${itemIdx}_${rowIdx}" 
           placeholder="0" 
           value="0" 
           step="1" 
           min="0"
           data-weight-per-package="${weightPerPackage}"
           data-item-idx="${itemIdx}"
           data-row-idx="${rowIdx}">
    <small class="text-muted d-block">กระป๋อง</small>
  </td>
  <td>
    <input class="form-control text-end lot-qty-calculated" 
           name="lot_qty_${itemIdx}_${rowIdx}" 
           value="0" 
           readonly
           style="background:#e8f5e9">
    <small class="text-success d-block">kg (อัตโนมัติ)</small>
  </td>
` : `
  <!-- 🔥 แสดงฟิลด์จำนวนปกติ ถ้าไม่มี package - ไม่ใช้ colspan -->
  <td>
    <input class="form-control text-end lot-qty" 
           name="lot_qty_${itemIdx}_${rowIdx}" 
           value="0" 
           step="0.0001" 
           min="0"
...
<!-- 🔥 ช่อง Pallet แยกออกมาเสมอ (ทั้งกรณีมีและไม่มี package) -->
...
function addLotRow(itemIdx){
  ...
  // ✅ ดึงค่าวันผลิต/วันหมดอายุจากแถวแรก (ถ้ามี)
  let mfgDate = '';
  let expDate = '';
  const firstRow = body.querySelector('tr[data-row="0"]');
  if (firstRow) {
    const mfgInput = firstRow.querySelector(`input[name*="lot_mfg"]`);
    const expInput = firstRow.querySelector(`input[name*="lot_exp"]`);
    if (mfgInput) mfgDate = mfgInput.value || '';
    if (expInput) expDate = expInput.value || '';
    console.log(`📅 Copy dates from row 0: MFG=${mfgDate}, EXP=${expDate}`);
  }
  
  body.insertAdjacentHTML('beforeend', renderLotRow(itemIdx, next));
  
  // ✅ เติมค่าวันผลิต/วันหมดอายุลงในแถวใหม่
  const newRow = body.querySelector(`tr[data-row="${next}"]`);
  if (newRow && mfgDate) {
    const newMfgInput = newRow.querySelector(`input[name*="lot_mfg"]`);
    if (newMfgInput) {
      newMfgInput.value = mfgDate;
      console.log(`✅ Set MFG date for row ${next}: ${mfgDate}`);
    }
  }
  if (newRow && expDate) {
    const newExpInput = newRow.querySelector(`input[name*="lot_exp"]`);
    if (newExpInput) {
      newExpInput.value = expDate;
      console.log(`✅ Set EXP date for row ${next}: ${expDate}`);
    }
  }
  
  hydrateLocationSelect(body.querySelector(`tr[data-row="${next}"] .lot-location`));
  
  // 🔥 เพิ่มส่วนนี้: คำนวณน้ำหนักจากกระป๋อง
  setupLotPackageCalculation(itemIdx, next);
  
  wireLotRowEvents(itemIdx);
  recalcLotSum(itemIdx);
}
// 🔥 ฟังก์ชันใหม่: คำนวณน้ำหนักจากจำนวนกระป๋อง
function setupLotPackageCalculation(itemIdx, rowIdx) {
  const row = document.querySelector(`tr[data-item="${itemIdx}"][data-row="${rowIdx}"]`);
  if (!row) {
    console.warn(`Cannot find row: item=${itemIdx}, row=${rowIdx}`);
    return;
  }
  
  const packageQtyInput = row.querySelector('.lot-package-qty');
  const calculatedQtyInput = row.querySelector('.lot-qty-calculated');
  
  if (!packageQtyInput || !calculatedQtyInput) {
    console.log(`Item ${itemIdx}, Row ${rowIdx}: No package mode (normal mode)`);
    return;
  }
  
  console.log(`Setup package calculation for item ${itemIdx}, row ${rowIdx}`);
  
  // ดึง weight_per_package จาก data attribute
  const weightPerPackage = parseFloat(packageQtyInput.dataset.weightPerPackage) || 0;
  
  if (weightPerPackage <= 0) {
    console.error(`Invalid weight_per_package: ${weightPerPackage}`);
    return;
  }
  
  console.log(`Weight per package: ${weightPerPackage} kg`);
  
  // ✅ ลบ event listener เดิมก่อน (ป้องกันการผูกซ้ำ)
  const newInput = packageQtyInput.cloneNode(true);
  packageQtyInput.parentNode.replaceChild(newInput, packageQtyInput);
  
  // ✅ ผูก event listener ใหม่
  newInput.addEventListener('input', function() {
    const packageQty = parseFloat(this.value) || 0;
    const totalWeight = packageQty * weightPerPackage;
    
    console.log(`Calculation: ${packageQty} × ${weightPerPackage} = ${totalWeight} kg`);
    
    // อัพเดทน้ำหนัก
    calculatedQtyInput.value = totalWeight.toFixed(4);
    
    // Visual feedback
    if (packageQty > 0) {
      calculatedQtyInput.style.background = '#c8e6c9';
      calculatedQtyInput.style.borderColor = '#4caf50';
    } else {
      calculatedQtyInput.style.background = '#e8f5e9';
      calculatedQtyInput.style.borderColor = '#ccc';
    }
  ...
  // ✅ Trigger calculation ครั้งแรกถ้ามีค่าอยู่แล้ว
  if (newInput.value && parseFloat(newInput.value) > 0) {
...
function recalcLotSum(itemIdx){
  ...
  let packageSum = 0;
const card = document.querySelector(`.po-item-card[data-item="${itemIdx}"]`);
const hasPackageInputs = body.querySelectorAll('.lot-package-qty').length > 0;
const packageInputs = body.querySelectorAll('.lot-package-qty');

// รวมกระป๋อง
if (hasPackageInputs) {
  packageInputs.forEach(i => {
    packageSum += parseFloat(i.value || '0');
  });
}
  let sum = 0, pallet = 0;
  
  // รวมจำนวนจากทุกแถว
  const qtyInputs = body.querySelectorAll('.lot-qty, .lot-qty-calculated');
  ...
  const displayValue = hasPackageInputs ? packageSum : sum;
  const checkValue = displayValue;
  if (sumEl) {
    sumEl.textContent = Math.round(displayValue).toLocaleString('th-TH');
  }
  if (palletSumEl) {
    palletSumEl.textContent = Math.round(pallet).toLocaleString('th-TH'); // ✅ แสดงผลรวม pallet
  }



  // ตรวจสอบว่าผลรวมไม่เกินจำนวนที่รับ
  const maxQty = parseFloat(card?.getAttribute('data-order-qty') || '0');
  
  console.log(`Item ${itemIdx}: displayValue=${displayValue}, maxQty=${maxQty}`);

  // ✅ คำนวณคงเหลือโดยตรง (ไม่ต้องแปลงหน่วยเพราะ data-order-qty อยู่ในหน่วยเดียวกับหน่วยรับแล้ว)
  const remaining = maxQty - displayValue;
  ...
  if (remaining < -0.5 && maxQty > 0) { // เกิน
    ...
    if (sumEl) {
      ...
      sumEl.innerHTML = `${Math.round(displayValue).toLocaleString('th-TH')} <i class="fa fa-exclamation-triangle ms-1" title="เกินจำนวนที่รับ ${Math.round(diff)}"></i>`;
    }
    ...
    warningBadge.innerHTML = `
      ...
      รวม: <span class="fw-bold">${Math.round(displayValue)}</span>
  ...
  } else if (remaining > 0.5 && maxQty > 0) { // ยังขาด
    
    if (sumEl) {
      sumEl.style.color = '#f59e0b';
      sumEl.style.fontWeight = 'bold';
      sumEl.innerHTML = `${Math.round(displayValue).toLocaleString('th-TH')} <i class="fa fa-info-circle ms-1" title="ยังขาด ${Math.round(remaining)}"></i>`;
    }
    ...
    warningBadge.innerHTML = `
      ...
      รวม: <span class="fw-bold">${Math.round(displayValue)}</span>
      | ต้องรับ: <span class="fw-bold">${Math.round(maxQty)}</span> 
      | ยังเหลือ: <span class="fw-bold text-warning">${Math.round(remaining)}</span>
    `;
    warningBadge.style.display = 'block';
    ...
    if (sumEl) {
      ...
      sumEl.style.fontWeight = 'bold';
      sumEl.innerHTML = `${Math.round(displayValue).toLocaleString('th-TH')} <i class="fa fa-check-circle ms-1 text-success" title="ครบถ้วน"></i>`;
    }
  ...
  console.log(`Item ${itemIdx} - displayValue: ${displayValue}, maxQty: ${maxQty}, remaining: ${remaining}, pallet: ${pallet}`);
}
...
// ===== แสดงรายการสินค้าแบบการ์ด (แก้ไขเพื่อป้องกันการแก้ไขสินค้าที่รับครบแล้ว) =====
function displayPOItemsAsCards(items){
  ...
  items.forEach((it, idx)=>{
    const hasPackage = it.package_qty && it.weight_per_package;
    
    const orderQtyOriginal = parseFloat(it.quantity || 0);
    const orderWeight = orderQtyOriginal;
    
    const orderQty = hasPackage 
        ? parseFloat(it.package_qty || 0)
        : orderQtyOriginal;
    ...
    const isFullyReceived = receivedQty >= orderQty && orderQty > 0;
    ...
    const purchaseUnitSymbol = hasPackage
        ? 'กระป๋อง'
        : (it.purchase_unit_symbol || it.purchase_unit_name || it.stock_unit_symbol || it.stock_unit_name || 'หน่วย');
    ...
    if (isFullyReceived) {
      statusHtml = `<span class="badge bg-success">
        <i class="fas fa-check-circle me-1"></i>รับครบแล้ว
      </span>`;
    ...
    // 🔥 CSS สำหรับสินค้าที่รับครบแล้ว
    const cardStyle = isFullyReceived 
      ? 'border:2px solid #059669;background:#f0fdf4;opacity:0.7;pointer-events:none;' 
      : 'border:2px solid rgba(139,69,19,.1);background:#fff;';
    
    const disabledAttr = isFullyReceived ? 'disabled' : '';
    const readonlyAttr = isFullyReceived ? 'readonly' : '';
    
    html += `
      <div class="po-item-card ${isFullyReceived ? 'fully-received' : ''}" 
           data-item="${idx}" 
           data-purchase-unit-id="${purchaseUnitId}" 
           data-order-qty="${orderQty}"
           data-order-qty-original="${orderQtyOriginal}"
           data-conversion-factor="${it.conversion_factor || 1}"
           data-fully-received="${isFullyReceived}"
           style="${cardStyle}border-radius:16px;padding:16px;margin-bottom:18px;position:relative">
        
        ${isFullyReceived ? `
        <!-- 🔥 Overlay แสดงว่ารับครบแล้ว -->
        <div style="position:absolute;top:10px;right:10px;z-index:10">
          <span class="badge bg-success" style="font-size:1rem;padding:0.5rem 1rem">
            <i class="fas fa-check-circle me-2"></i>รับครบแล้ว - ไม่สามารถแก้ไขได้
          </span>
        </div>
        ` : ''}
        ...
        <div class="po-item-head" style="display:flex;gap:16px;align-items:flex-start;justify-content:space-between">
          <div style="flex:1">
            <div class="d-flex align-items-center gap-2 mb-2">
              <div class="form-check d-inline-block me-2" style="margin-top: 2px;">
                <input class="form-check-input item-select-checkbox" 
                       type="checkbox" 
                       id="select_item_${idx}" 
                       data-item="${idx}" 
                       ${isFullyReceived ? 'disabled checked' : 'checked'}
                       onchange="toggleItemReceiving(${idx})" 
                       style="cursor: ${isFullyReceived ? 'not-allowed' : 'pointer'}; width: 20px; height: 20px;">
                <label class="form-check-label" for="select_item_${idx}" style="cursor: ${isFullyReceived ? 'not-allowed' : 'pointer'};">
                  <small class="text-muted">รับ</small>
                </label>
              </div>
            ...
            <div class="po-item-meta">
              ...
              <small class="text-primary fw-bold">
                <i class="fa fa-shopping-cart me-1"></i>สั่งซื้อ: ${
                  orderQty > 0 
                    ? hasPackage
                      ? `<span class="fs-6">${numberFormat(orderQty)}</span> ${hEsc(purchaseUnitSymbol)} <span class="text-muted">(${numberFormat(orderWeight)} kg รวม)</span>`
                      : `<span class="fs-6">${numberFormat(orderQty)}</span> ${hEsc(purchaseUnitSymbol)}`
              ...
              ${orderQty > 0 && receivedQty < orderQty ? `<small class="text-warning"><i class="fa fa-clock me-1"></i>ค้างรับ: ${numberFormat(orderQty - receivedQty)} ${hEsc(purchaseUnitSymbol)}</small><br/>` : ''}
          ...
          ${!it.package_qty || !it.weight_per_package ? `
            <div style="min-width:450px;background:#f8f9fa;padding:15px;border-radius:10px">
              <div class="row g-2 align-items-center mb-3">
                ...
                <div class="col-8">
                  <select class="form-select recv-unit" name="recv_unit_${idx}" ${disabledAttr}>
              ...
              <div class="row g-2 align-items-center">
                ...
                <div class="col-8">
                  <input class="form-control text-end recv-qty fw-bold" 
                         name="recv_qty_${idx}" 
                         placeholder="0" 
                         step="1" 
                         min="0" 
                         ${readonlyAttr}
                         style="font-size:1.2rem;color:#059669;background:${isFullyReceived ? '#e8f5e9' : 'white'}" />
                  <small class="text-muted d-block mt-1">
                    <i class="fa fa-info-circle me-1"></i>
                    ${isFullyReceived ? 'รับครบแล้ว - ไม่สามารถแก้ไข' : 'คนละหน่วยกับการสั่งซื้อ'}
                  </small>
                </div>
              </div>
              ...
              <div class="mt-2 p-2" style="background:white;border-radius:8px;min-height:40px">
                <small class="badge-note d-block" id="conv_note_${idx}" style="font-size:.82rem;color:#059669;font-weight:500">
                  ${isFullyReceived ? '<i class="fa fa-check-circle me-1"></i>รับครบแล้ว' : '<i class="fa fa-calculator me-1"></i>รอเลือกหน่วยรับ...'}
                </small>
              </div>
            </div>
          ` : `
            <input type="hidden" class="recv-unit" name="recv_unit_${idx}" value="${purchaseUnitId}">
            <input type="hidden" class="recv-qty" name="recv_qty_${idx}" value="0">
          `}
        </div>

        ${isFullyReceived ? `
        <!-- 🔥 แสดงข้อมูลที่รับไว้แล้ว (Read-only) -->
        <div class="alert alert-success mt-3 mb-3">
          <i class="fas fa-info-circle me-2"></i>
          <strong>สินค้ารายการนี้รับครบแล้ว</strong><br>
          <small>รับไปแล้ว: <strong>${numberFormat(receivedQty)} ${hEsc(purchaseUnitSymbol)}</strong> จาก ${numberFormat(orderQty)} ${hEsc(purchaseUnitSymbol)}</small>
        </div>
        ` : `
        <!-- 🔥 แสดงฟอร์มกระจายล็อตเฉพาะสินค้าที่ยังรับไม่ครบ -->
              ...
              <tr>
                ...
                ${it.package_qty && it.weight_per_package ? `
                  <th style="width:8%" class="text-end">
                    <i class="fas fa-box text-primary me-1"></i>กระป๋อง
                  </th>
                  <th style="width:10%" class="text-end">
                    <i class="fas fa-weight text-success me-1"></i>น้ำหนัก (kg)
                  </th>
                ` : `
              ...
              <tr class="table-light">
                ...
                <td colspan="5">
                  <div class="d-flex justify-content-between align-items-center">
                    <div>
                      <small class="text-primary fw-bold">
                        <i class="fa fa-clipboard-check me-1"></i>ต้องรับ: ${Math.round(orderQty - receivedQty).toLocaleString()} ${hEsc(purchaseUnitSymbol)}
                      </small>
                    </div>
                    <div>
                      <small class="me-2">
                        ...
                        <span id="remaining_${idx}" class="fw-bold text-warning">
                          ${Math.round(orderQty - receivedQty).toLocaleString()}
                        </span>
                      </small>
                    </div>
                  </div>
  ...
  const itemSelectionButtons = document.getElementById('itemSelectionButtons');
  if (itemSelectionButtons && items.length > 1) {
    itemSelectionButtons.style.display = 'block';
  }
  ...
  items.forEach((it, idx)=> {
    const isFullyReceived = parseFloat(it.received_quantity || 0) >= parseFloat(it.quantity || 0);
    
    if (!isFullyReceived) {
      wireItemCardEvents(it, idx);
      
      setTimeout(() => {
        setupLotPackageCalculation(idx, 0);
      }, 100);
    }
  });
...
function wireItemCardEvents(item, idx){
 ...
 // 🔥 เพิ่มส่วนนี้: คำนวณน้ำหนักจากจำนวนกระป๋อง
  const packageQtyInput = card.querySelector('.recv-package-qty');
  const totalWeightInput = card.querySelector('.recv-total-weight');
  
  if (packageQtyInput && totalWeightInput) {
    console.log(`Item ${idx}: Package mode enabled`);
    
    packageQtyInput.addEventListener('input', function() {
      const packageQty = parseFloat(this.value) || 0;
      const weightPerPackage = parseFloat(this.dataset.weightPerPackage) || 0;
      const totalWeight = packageQty * weightPerPackage;
      ...
      // Visual feedback
      if (packageQty > 0) {
        totalWeightInput.style.background = '#c8e6c9';
        totalWeightInput.style.borderColor = '#4caf50';
      } else {
        totalWeightInput.style.background = '#e8f5e9';
      ...
      console.log(`Package calc: ${packageQty} × ${weightPerPackage} = ${totalWeight} kg`);
    });
  }
  ...
  unitEl.addEventListener('change', async function() {
      ...
      if (purchaseUnitId && orderQty > 0) {
        ...
        try {
          ...
          if (convertedQty !== null && convertedQty > 0) {
            ...
            // ✅ อัพเดท data-order-qty ให้ตรงกับหน่วยรับ
            card.dataset.orderQty = convertedQty;
            console.log(`✅ Updated data-order-qty for item ${idx}: ${convertedQty} (in recv unit ${newUnitId})`);
    ...
    // ✅ กรณีที่ 2: มีค่าอยู่แล้ว และเปลี่ยนหน่วย - แปลงค่าเดิม
      ...
      try {
        // ✅ แปลง orderQty ดั้งเดิมเป็นหน่วยใหม่
        const orderQtyOriginal = parseFloat(card.dataset.orderQtyOriginal || orderQty);
        const convertedOrderQty = await convertQtyByIds(item.product_id, purchaseUnitId, newUnitId, orderQtyOriginal);
        
        if (convertedOrderQty !== null && convertedOrderQty > 0) {
          card.dataset.orderQty = convertedOrderQty;
          console.log(`✅ Updated data-order-qty for item ${idx}: ${convertedOrderQty} (changed to unit ${newUnitId})`);
        }
  ...
  if (initialUnitId && purchaseUnitId && orderQty > 0) {
    ...
    try {
      ...
      if (convertedQty !== null && convertedQty > 0) {
        ...
        // ✅ อัพเดท data-order-qty ให้ตรงกับหน่วยรับ
        card.dataset.orderQty = convertedQty;
        console.log(`✅ Initial load - Updated data-order-qty for item ${idx}: ${convertedQty} (in unit ${initialUnitId})`);
...
function collectItemPayload(idx, item){
  const card = document.querySelector(`.po-item-card[data-item="${idx}"]`);
  const qtyEl = card.querySelector('.recv-qty, .recv-total-weight');
  const unitEl = card.querySelector('.recv-unit');
  // รวมจำนวนกระป๋องจากทุก lot
let totalPackageQty = 0;
card.querySelectorAll('.lot-package-qty').forEach(input => {
    totalPackageQty += parseFloat(input.value || 0);
});

  const lots = [];
  let totalPallet = 0;
  let totalQty = 0; // 🔥 เพิ่ม: รวมจำนวนจากล็อต
  
  card.querySelectorAll('#lot_body_'+idx+' tr').forEach(tr=>{
    const qtyInput = tr.querySelector('.lot-qty, .lot-qty-calculated');
    const qty = parseFloat(qtyInput?.value || '0') || 0;
    const pallet = parseFloat(tr.querySelector('.lot-pallet')?.value || '0') || 0;
    
    totalPallet += pallet;
    totalQty += qty; // 🔥 เพิ่ม: สะสมจำนวน
    
    lots.push({
      location_id: tr.querySelector('.lot-location')?.value || null,
      lot_number: tr.querySelector(`[name^="lot_no_${idx}_"]`)?.value || '',
      quantity: qty,
      pallet_count: pallet,
      mfg_date: tr.querySelector(`[name^="lot_mfg_${idx}_"]`)?.value || null,
      exp_date: tr.querySelector(`[name^="lot_exp_${idx}_"]`)?.value || null,
      condition: tr.querySelector(`[name^="lot_cond_${idx}_"]`)?.value || '',
      note: tr.querySelector(`[name^="lot_note_${idx}_"]`)?.value || ''
    });
  });

  // 🔥 แก้ไข: ใช้ totalQty ถ้า qtyEl ไม่มีค่า (กรณีหมึก)
  const receiveQty = parseFloat(qtyEl?.value || '0') || totalQty;

  return {
    po_item_id: item.po_item_id,
    product_id: item.product_id,
    receive_qty: receiveQty, // 🔥 แก้ไข
    unit_id: parseInt(unitEl?.value||'0')||null,
    pallet_count: totalPallet,
    recv_package_qty: totalPackageQty > 0 ? totalPackageQty : null,
    locations: lots.filter(l => l.quantity > 0)
  };
}


// ===== เพิ่มฟังก์ชันตรวจสอบก่อนบันทึก =====
// ===== แก้ไข validateReceiptData =====
function validateReceiptData() {
  ...
  itemCards.forEach((card, idx) => {
    // ✅ ตรวจสอบว่าสินค้านี้เลือกไว้หรือไม่
    const checkbox = document.getElementById(`select_item_${idx}`);
    if (checkbox && !checkbox.checked) {
      return; // ข้ามสินค้าที่ไม่ได้เลือก
    }
    
    hasSelectedItems++;
    
    const qtyEl = document.querySelector(`[name="recv_qty_${idx}"]`);
    let maxQty = parseFloat(qtyEl?.value || '0');
    
    if (maxQty <= 0) {
      const lotInputs = document.querySelectorAll(`#lot_body_${idx} .lot-qty, #lot_body_${idx} .lot-qty-calculated`);
      lotInputs.forEach(inp => {
        maxQty += parseFloat(inp.value || '0');
      });
    }
    ...
    // ✅ ดึงชื่อสินค้า
    const productName = card.querySelector('.po-item-title')?.textContent?.trim() || `รายการที่ ${idx + 1}`;
    
    lotInputs.forEach((inp, rowIdx) => {
      const val = parseFloat(inp.value || '0');
      lotSum += val;
      
      const row = inp.closest('tr');
      const locationSelect = row?.querySelector('.lot-location');
      const lotInput = row?.querySelector(`[name^="lot_no_${idx}_"]`);
      const lotNumber = lotInput?.value?.trim() || '';
      
      // ✅ เช็คว่ามี location และมีจำนวน
      if (locationSelect && locationSelect.value && val > 0) {
        hasLocation = true;
        
        // 🔥 บังคับให้ใส่ Lot Number
        if (!lotNumber) {
          rowsWithoutLot.push(rowIdx + 1);
        }
      }
    });
    ...
    // 🔥 บังคับให้ใส่ Lot Number
    if (rowsWithoutLot.length > 0) {
      errors.push(`❌ ${productName}: กรุณาใส่เลข Lot ในแถวที่ ${rowsWithoutLot.join(', ')}`);
    }
  });
  
  // ✅ ตรวจสอบว่ามีรายการที่เลือกหรือไม่
  if (hasSelectedItems === 0) {
    errors.push('กรุณาเลือกรายการสินค้าที่ต้องการรับอย่างน้อย 1 รายการ');
  }
...
function submitReceiptEnhancedPO(){
  console.log('🔍 Starting validation...'); // 🔥 เพิ่ม debug
  
  // ตรวจสอบข้อมูล
  const validation = validateReceiptData();
  
  console.log('Validation result:', validation); // 🔥 เพิ่ม debug
  
  // แสดง errors
  if (validation.errors.length > 0) {
    console.log('❌ Validation errors found:', validation.errors); // 🔥 เพิ่ม debug
    
    const errorMsg = '<strong>❌ ไม่สามารถบันทึกได้:</strong><br><ul class="mb-0 ps-3">' + 
                     validation.errors.map(e => `<li>${e}</li>`).join('') + 
                     '</ul><small class="text-muted d-block mt-2">กรุณาแก้ไขข้อมูลก่อนดำเนินการต่อ</small>';
    
    const alertDiv = document.createElement('div');
    alertDiv.className = 'alert alert-danger alert-dismissible fade show';
    alertDiv.style.cssText = 'position: fixed; top: 20px; left: 50%; transform: translateX(-50%); z-index: 9999; min-width: 500px; box-shadow: 0 4px 20px rgba(0,0,0,0.3);';
    alertDiv.innerHTML = `
    ...
    // ✅ เพิ่ม: ซ่อนอัตโนมัติหลัง 10 วินาที
    setTimeout(() => {
      alertDiv.remove();
    }, 10000);
  ...
  if (validation.warnings.length > 0) {
    console.log('⚠️ Validation warnings found:', validation.warnings); // 🔥 เพิ่ม debug
    
    const warningMsg = '⚠️ มีข้อควรระวัง:\n\n' + 
  ...
  console.log('✅ Validation passed, proceeding to save...'); // 🔥 เพิ่ม debug
  ...
  const items_data = window.currentItems
    .map((it, idx)=> {
      const checkbox = document.getElementById(`select_item_${idx}`);
      if (checkbox && !checkbox.checked) {
        return null;
      }
      return collectItemPayload(idx, it);
    })
    .filter(item => item !== null && item.receive_qty > 0);
  
  if (items_data.length === 0) {
    alert('กรุณาเลือกสินค้าที่ต้องการรับอย่างน้อย 1 รายการ และกรอกจำนวนให้ครบถ้วน');
    return;
  }
  
  console.log('📦 Items to save:', items_data); // 🔥 เพิ่ม debug
  ...
  const payload = {
    ...
    items_data: items_data.map(item => ({
      po_item_id: item.po_item_id,
      product_id: item.product_id,
      ordered_qty: item.receive_qty,
      receive_qty: item.receive_qty,
      unit_id: item.unit_id,
      recv_package_qty: item.recv_package_qty,
      unit_price: 0,
      total_price: 0,
      lot_number: item.locations[0]?.lot_number || null,
      mfg_date: item.locations[0]?.mfg_date || null,
      exp_date: item.locations[0]?.exp_date || null,
      pallet_count: item.pallet_count || 0,
      locations: item.locations
    }))
  };

  console.log('📤 Sending payload:', payload); // 🔥 เพิ่ม debug
  ...
  .then(res => {
    hideLoading();
    console.log('📥 Response:', res); // 🔥 เพิ่ม debug
