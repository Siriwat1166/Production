<?php
// pages/material_types/edit.php - แก้ไขประเภทวัสดุ
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // ตรวจสอบรหัสประเภทซ้ำ (ยกเว้นตัวเอง)
        $checkStmt = $conn->prepare("SELECT COUNT(*) FROM Material_Types WHERE type_code = ? AND material_type_id != ?");
        $checkStmt->execute([$type_code, $material_type_id]);
        if ($checkStmt->fetchColumn() > 0) {
            throw new Exception("รหัสประเภท '$type_code' มีอยู่แล้วในระบบ");
        }
        
        // ตรวจสอบชื่อประเภทซ้ำ (ยกเว้นตัวเอง)
        $checkNameStmt = $conn->prepare("SELECT COUNT(*) FROM Material_Types WHERE type_name = ? AND material_type_id != ?");
        $checkNameStmt->execute([$type_name, $material_type_id]);
        ...
        $stmt = $conn->prepare("
            UPDATE Material_Types SET 
                type_code = ?, type_name = ?, description = ?, is_active = ?,
                updated_by = ?, updated_date = GETDATE()
            WHERE material_type_id = ?
        ");
        
        $stmt->execute([
            $type_code, $type_name, $description, $is_active, $user_id, $material_type_id
        ]);
        
        $message = "อัปเดตข้อมูลประเภทวัสดุ '$type_name' เรียบร้อยแล้ว";
        $message_type = "success";
        
        // อัปเดตข้อมูลในตัวแปร material_type
        $material_type['type_code'] = $type_code;
        $material_type['type_name'] = $type_name;
...
<head>
    ...
    <title>แก้ไขประเภทวัสดุ - <?= defined('APP_NAME') ? APP_NAME : 'Material Management System' ?></title>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="mb-0">
                            <i class="fas fa-edit me-2"></i>แก้ไขประเภทวัสดุ
                        </h4>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-8">
                                <p class="text-muted mb-0">
                                    ...
                                    แก้ไขข้อมูลประเภทวัสดุ: <strong><?= htmlspecialchars($material_type['type_name']) ?></strong>
                                </p>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control" id="type_code" name="type_code" 
                                           value="<?= htmlspecialchars($material_type['type_code']) ?>" 
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control" id="type_name" name="type_name" 
                                           value="<?= htmlspecialchars($material_type['type_name']) ?>" 
                            ...
                            <div class="row">
                                <div class="col-md-12 mb-3">
                                    ...
                                    <textarea class="form-control" id="description" name="description" rows="3" 
                                              ...
                                              maxlength="500"><?= htmlspecialchars($material_type['description'] ?? '') ?></textarea>
                                    <div class="form-text">
                                        <i class="fas fa-info-circle me-1"></i>
                                ...
                                <div class="col-md-12 mb-3">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" id="is_active" name="is_active" 
                                               <?= $material_type['is_active'] ? 'checked' : '' ?>>
                                        <label class="form-check-label" for="is_active">
                                            เปิดใช้งานประเภทวัสดุนี้
                                        </label>
                                    </div>
                                    <div class="form-text">
                                        <i class="fas fa-toggle-on me-1"></i>
                                        หากปิดการใช้งาน ประเภทวัสดุนี้จะไม่ปรากฏในรายการเลือก
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card-body">
                            <div class="preview-box">
                                ...
                                <div id="previewCode" class="fw-bold text-primary">
                                    <span id="codePreview"><?= htmlspecialchars($material_type['type_code']) ?></span>
                                </div>
                                <div id="previewName" class="fw-bold">
                                    <span id="namePreview"><?= htmlspecialchars($material_type['type_name']) ?></span>
                                </div>
                                <small class="text-muted">
                                    <span id="descPreview"><?= htmlspecialchars($material_type['description'] ?? 'รายละเอียด') ?></span>
                                </small>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="card-body">
                            <table class="table table-sm">
                                ...
                                <tr>
                                    ...
                                    <td><?= date('d/m/Y H:i', strtotime($material_type['created_date'])) ?></td>
                                </tr>
                                <tr>
                                    <td><strong>สร้างโดย:</strong></td>
                                    <td>User ID: <?= $material_type['created_by'] ?></td>
                                </tr>
                                <tr>
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="d-grid gap-2">
                                ...
                                <a href="view.php?id=<?= $material_type['material_type_id'] ?>" class="btn btn-secondary">
